...
/proc/sanitizeSQL(var/input as text, var/max_length_char = MAX_MESSAGE_LEN)
	if (!input)
		input = ""
	...
	if (length_char(input) > max_length_char)
		input = copytext_char(input, TRUE, max_length_char+1)
	return input
/*
 * Text sanitization
 */
...
/proc/sanitize(var/input, var/max_length_char = MAX_MESSAGE_LEN, var/encode = TRUE, var/trim = TRUE, var/extra = TRUE)
	if (!input)
		return
...
/proc/sanitizeSafe(var/input, var/max_length_char = MAX_MESSAGE_LEN, var/encode = TRUE, var/trim = TRUE, var/extra = TRUE)
	return sanitize(replace_characters(input, list(">"=" ","<"=" ", "\""="'")), max_length_char, encode, trim, extra)

//Filters out undesirable characters from names
/proc/sanitizeName(var/input, var/max_length_char = MAX_NAME_LEN, var/allow_numbers = FALSE)
...
/proc/trim_left(text)
	for (var/i = TRUE to length_char(text))
...
/proc/stringmerge(var/text,var/compare,replace = "*")
	...
	if (length_char(text) != length_char(compare))
		return FALSE
	for (var/i = TRUE, i < length_char(text), i++)
