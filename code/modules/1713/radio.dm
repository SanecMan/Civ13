...
/proc/ten_code(message,mob/living/human/speaker)
	...
	switch(tcode)
		if ("0")
			converted = TRUE
			dmessage = "10-0: В пути, еду в [speaker.get_coded_loc(speaker)] ([speaker.x],[speaker.y])."
		if ("1")
			converted = TRUE
			dmessage = "10-1: Сообщаю, я нахожусь [speaker.get_coded_loc(speaker)] ([speaker.x],[speaker.y])."
		if ("2")
			converted = TRUE
			dmessage = "10-2: Сообщаю, свободен."
		if ("3")
			converted = TRUE
		...
		if ("5")
			converted = TRUE
			dmessage = "10-5: Отказано!"
		if ("6")
			converted = TRUE
			dmessage = "10-6: Возращаюсь."
		if ("7")
			converted = TRUE
			dmessage = "10-7: Задержан подозреваемый."
		if ("8")
			converted = TRUE
			dmessage = "10-8: НЕОБХОДИМА СРОЧНАЯ ПОМОЩЬ В [speaker.get_coded_loc()] ([speaker.x],[speaker.y])!"
